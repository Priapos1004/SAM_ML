...
def synt_data(columns: dict[list], label_name: str = "label", sample_num: int = 1000, classes_num: int = 2, weights: list[float] = None, class_sep: float = 0.98) -> pd.DataFrame:
    """
    @param:
    ...
    X, y = make_classification(n_samples=sample_num, n_features=total_feature_num, n_informative = len(informativ_features), n_redundant=0, n_repeated=0,
                            class_sep=class_sep, random_state=42, weights = weights, n_classes=classes_num)
